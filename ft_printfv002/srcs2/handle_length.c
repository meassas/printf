...
/*   Updated: 2017/09/10 19:36:19 by meassas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		print_type(t_format *format, t_conversion *conversion, va_list arguments)
{
	if (conversion->specifier == SIGNED_DECIMAL)
		...
		handle_pointer(format, conversion, va_arg(arguments, void*));
	else if (conversion->specifier == STRING)
		handle_string(format, conversion, va_arg(arguments, char*));
	else if (conversion->specifier == CHAR)
		handle_char(format, conversion, va_arg(arguments, int));
}
